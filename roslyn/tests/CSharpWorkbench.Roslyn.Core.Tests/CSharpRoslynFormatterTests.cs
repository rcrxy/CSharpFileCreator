...
namespace CSharpWorkbench.Roslyn.Core.Tests;
...
public sealed class CSharpRoslynFormatterTests
{
    ...
    {
        var invalidSpan = await Assert.ThrowsAsync<CSharpFormattingException>(() =>
            new CSharpRoslynFormatter().FormatAsync(
        ...
        var invalidConfiguration = await Assert.ThrowsAsync<CSharpFormattingException>(() =>
            new CSharpRoslynFormatter().FormatAsync(
                new CSharpFormattingRequest("class Demo { }", CSharpFormattingKind.Document, invalidOptions)));

        Assert.Equal(CSharpFormattingErrorCode.InvalidSpan, invalidSpan.Code);
        Assert.Equal(CSharpFormattingErrorCode.InvalidConfiguration, invalidConfiguration.Code);
    }

    [Fact]
    public async Task FormatAsyncRejectsUnsupportedSnippetKind()
    {
        var exception = await Assert.ThrowsAsync<CSharpFormattingException>(() =>
            new CSharpRoslynFormatter().FormatAsync(
                new CSharpFormattingRequest(
                    "Work();",
                    CSharpFormattingKind.Snippet,
        ...
        Assert.Equal(CSharpFormattingErrorCode.InvalidRequest, exception.Code);
    }
    ...
    public void CoreInfoReportsVersionAndCapabilityBoundaries()
    {
        ...
        Assert.Equal("roslyn-core", info.BackendName);
        ...
        Assert.False(info.Capabilities.SupportsIndependentEventIndexerAndLocalFunctionBraceContexts);
